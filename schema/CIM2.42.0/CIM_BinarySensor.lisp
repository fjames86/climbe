...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_BinarySensor.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_BINARYSENSOR (CIM_SENSOR)
              ((CURRENTREADING BOOLEAN :CIM-NAME "CurrentReading" :QUALIFIERS
                ((:DEPRECATED ("CIM_Sensor.SensorType"))
                 (:DESCRIPTION "The current value indicated by the Sensor."))
                :INITFORM NIL)
               (EXPECTEDREADING BOOLEAN :CIM-NAME "ExpectedReading" :QUALIFIERS
                ((:DEPRECATED ("CIM_Sensor.SensorType"))
                 (:DESCRIPTION
                  "ExpectedReading indicates the 'normal' value for the Sensor."))
                :INITFORM NIL)
               (INTERPRETATIONOFTRUE STRING :CIM-NAME "InterpretationOfTrue"
                :QUALIFIERS
                ((:DEPRECATED ("CIM_Sensor.PossibleStates"))
                 ...
                 (:MAX-LEN "64"))
                :INITFORM NIL)
               (INTERPRETATIONOFFALSE STRING :CIM-NAME "InterpretationOfFalse"
                :QUALIFIERS
                ((:DEPRECATED ("CIM_Sensor.PossibleStates"))
                 (:DESCRIPTION
...
OperationalStatus replaces the Status property on ManagedSystemElement to provide a consistent approach to enumerations, to address implementation needs for an array property, and to provide a migration path from today's environment to the future. This change was not made earlier because it required the deprecated qualifier. Due to the widespread use of the existing Status property in management applications, it is strongly recommended that providers or instrumentation provide both the Status and OperationalStatus properties. Further, the first value of OperationalStatus should contain the primary status for the element. When instrumented, Status (because it is single-valued) should also provide the primary status of the element.")
               ...
               (STATUSDESCRIPTIONS (ARRAY STRING) :CIM-NAME
                ...
                :INITFORM NIL))
              (:CIM-NAME "CIM_BinarySensor")
              (:QUALIFIERS (:DEPRECATED ("CIM_Sensor")) (:VERSION "2.8.0")
