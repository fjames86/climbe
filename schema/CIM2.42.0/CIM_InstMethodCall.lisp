...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_InstMethodCall.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_INSTMETHODCALL (CIM_INSTINDICATION)
              ((METHODNAME STRING :CIM-NAME "MethodName" :QUALIFIERS
                (:REQUIRED (:DESCRIPTION "The name of the method invoked."))
                :INITFORM NIL)
               (METHODPARAMETERS STRING :CIM-NAME "MethodParameters"
                :QUALIFIERS
                ((:DESCRIPTION
...
If PreCall is FALSE, the embedded instance contains only properties corresponding to the output parameters of the method, and their values are the parameter values after the method call.")
                 :EMBEDDED-OBJECT)
                :INITFORM NIL)
               (RETURNVALUE STRING :CIM-NAME "ReturnValue" :QUALIFIERS
                ((:DESCRIPTION
...
If PreCall is False, ReturnValue contains a string representation of the method's return value. REF-typed method return values shall be represented as an instance path in WBEM URI format")
                 (:MODEL-CORRESPONDENCE ("CIM_InstMethodCall.PreCall")))
                :INITFORM NIL)
               (PRECALL BOOLEAN :CIM-NAME "PreCall" :QUALIFIERS
                (:REQUIRED
                 (:DESCRIPTION
