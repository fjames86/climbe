...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_OpaqueManagementData.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_OPAQUEMANAGEMENTDATA (CIM_STORAGEEXTENT)
              ((MAXSIZE UINT64 :CIM-NAME "MaxSize" :QUALIFIERS
                ((:DESCRIPTION
                  "The maximum number of bytes of opaque management data that the instance may contain.")
                 (:P-UNIT "byte"))
                :INITFORM NIL)
               (LASTACCESSED DATETIME :CIM-NAME "LastAccessed" :QUALIFIERS
                ((:DESCRIPTION
                  "The date and time when the opaque management data was last accessed. Any operation on the OpaqueManagementData instance constitutes access. This property shall be NULL if it is not supported by the implementation."))
                :INITFORM NIL)
               (WRITELIMITED UINT16 :CIM-NAME "WriteLimited" :QUALIFIERS
                ((:DESCRIPTION
                  "Indicates whether the data are stored on a medium that has a limited number of write/erase cycles (e.g., flash memory).")
                 (:VALUE-MAP ("0" "1" "2" ".."))
                 (:VALUES
                  ("Unknown" "Not write-limited" "Write-limited"
                   "DMTF Reserved")))
                :INITFORM NIL)
               (DATAFORMAT STRING :CIM-NAME "DataFormat" :QUALIFIERS
                ((:DESCRIPTION
                  "Describes the format of the opaque management data. A free-form string that shall only be set by an authorized client, and shall not be changed by the implementation."))
                :INITFORM NIL)
               (DATASIZE UINT64 :CIM-NAME "DataSize" :QUALIFIERS
                ((:DESCRIPTION
...
OperationalStatus replaces the Status property on ManagedSystemElement to provide a consistent approach to enumerations, to address implementation needs for an array property, and to provide a migration path from today's environment to the future. This change was not made earlier because it required the deprecated qualifier. Due to the widespread use of the existing Status property in management applications, it is strongly recommended that providers or instrumentation provide both the Status and OperationalStatus properties. Further, the first value of OperationalStatus should contain the primary status for the element. When instrumented, Status (because it is single-valued) should also provide the primary status of the element.")
               ...
               (STATUSDESCRIPTIONS (ARRAY STRING) :CIM-NAME
                ...
                :INITFORM NIL))
              (:CIM-NAME "CIM_OpaqueManagementData")
...
(DEFCIM-METHOD SETPOWERSTATE
    ((INSTANCE CIM_OPAQUEMANAGEMENTDATA)
