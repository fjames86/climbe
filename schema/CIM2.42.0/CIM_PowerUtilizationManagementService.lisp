...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_PowerUtilizationManagementService.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_POWERUTILIZATIONMANAGEMENTSERVICE (CIM_SERVICE)
              ((POWERUTILIZATIONMODE UINT16 :CIM-NAME "PowerUtilizationMode"
                :QUALIFIERS
                ((:DESCRIPTION
                  "PowerUtilizationMode indicates the current power utilization mode implemented by the service. The mode selection is a generally made infrequently as part of element configuration. 
...
OperationalStatus replaces the Status property on ManagedSystemElement to provide a consistent approach to enumerations, to address implementation needs for an array property, and to provide a migration path from today's environment to the future. This change was not made earlier because it required the deprecated qualifier. Due to the widespread use of the existing Status property in management applications, it is strongly recommended that providers or instrumentation provide both the Status and OperationalStatus properties. Further, the first value of OperationalStatus should contain the primary status for the element. When instrumented, Status (because it is single-valued) should also provide the primary status of the element.")
               ...
               (STATUSDESCRIPTIONS (ARRAY STRING) :CIM-NAME
                ...
                :INITFORM NIL))
              (:CIM-NAME "CIM_PowerUtilizationManagementService")
...
(DEFCIM-METHOD APPLYPOWERALLOCATIONSETTINGS
    ((INSTANCE CIM_POWERUTILIZATIONMANAGEMENTSERVICE)
     (POWERALLOCATIONSETTINGS STRING "PowerAllocationSettings" :REQUIRED :IN
      (:DESCRIPTION
       "An embedded instance of class CIM_PowerAllocationSettingData that describes modifications to the existing power allocation aspects of a computer system. The embedded instance must have a valid InstanceID in order to identify the power allocation setting to be modified.")
      (:EMBEDDED-INSTANCE "CIM_PowerAllocationSettingData"))
     (JOB "CIM_ConcreteJob" "Job" :IN :OUT
      (:DESCRIPTION
       "If the operation is long running, then optionally a job be returned.")))
    ((:CIM-NAME "ApplyPowerAllocationSettings")
     (:QUALIFIERS
      (:DESCRIPTION
       "Applies the Power Allocation Settings that represents.the power aspect of the system. When applied to a system's Power Allocation settings, a system's power allocation settingsmay be modified.Upon successfull execution if the limit property of the power aspectis modified the power cap is modified")
