...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_PublicPrivateKeyAuthentication.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_PUBLICPRIVATEKEYAUTHENTICATION (CIM_AUTHENTICATIONCONDITION)
              ((SELFISSUEDKEY BOOLEAN :CIM-NAME "SelfIssuedKey" :QUALIFIERS
                ((:DESCRIPTION
                  "Boolean indicating whether the key pair is self-issued (TRUE) or issued by a Certificate Authority (FALSE)."))
                :INITFORM NIL)
               (DISTINGUISHEDNAME STRING :CIM-NAME "DistinguishedName"
                :QUALIFIERS
                ((:DESCRIPTION
                  "String holding the user's (distinguished) name."))
                :INITFORM NIL)
               (PUBLICKEY STRING :CIM-NAME "PublicKey" :QUALIFIERS
...
This MOF defines the following keywords: 'UNKNOWN', 'CONFIGURATION', 'USAGE', 'SECURITY', 'SERVICE', 'MOTIVATIONAL', 'INSTALLATION', and 'EVENT'. These concepts are self-explanatory and are further discussed in the SLA/Policy White Paper. One additional keyword is defined: 'POLICY'. The role of this keyword is to identify policy-related instances that may not be otherwise identifiable, in some implementations. The keyword 'POLICY' is NOT mutually exclusive of the other keywords specified above."))
                :INITFORM NIL))
              (:CIM-NAME "CIM_PublicPrivateKeyAuthentication")
