...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_RemoveDirectoryAction.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_REMOVEDIRECTORYACTION (CIM_DIRECTORYACTION)
              ((MUSTBEEMPTY BOOLEAN :CIM-NAME "MustBeEmpty" :QUALIFIERS
                ((:DESCRIPTION
                ...
                :INITFORM NIL)
               (DIRECTORYNAME STRING :CIM-NAME "DirectoryName" :QUALIFIERS
...
Note that the Name property of ManagedSystemElement is also defined as a user-friendly name. But, it is often subclassed to be a Key. It is not reasonable that the same property can convey both identity and a user-friendly name, without inconsistencies. Where Name exists and is not a Key (such as for instances of LogicalDevice), the same information can be present in both the Name and ElementName properties. Note that if there is an associated instance of CIM_EnabledLogicalElementCapabilities, restrictions on this properties may exist as defined in ElementNameMask and MaxElementNameLen properties defined in that class."))
                :INITFORM NIL))
              (:CIM-NAME "CIM_RemoveDirectoryAction")
...
(DEFCIM-METHOD INVOKE
    ((INSTANCE CIM_REMOVEDIRECTORYACTION))
    ((:CIM-NAME "Invoke")
     (:QUALIFIERS
      (:DESCRIPTION
