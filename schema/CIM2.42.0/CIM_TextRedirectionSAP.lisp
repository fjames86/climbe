...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_TextRedirectionSAP.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_TEXTREDIRECTIONSAP (CIM_SERVICEACCESSPOINT)
              ((SESSIONTERMINATESEQUENCE STRING :CIM-NAME
                "SessionTerminateSequence" :QUALIFIERS
                ((:DESCRIPTION
                  "A string sequence that allows the supervisor using the command line session to terminate the redirection stream. This is required because once the text redirection is started then the supervisor may lose the ability to interact with the command line session, and be unable to perform a RequestStateChange operation."))
                :INITFORM NIL)
               (TEXTFLOWTYPE UINT16 :CIM-NAME "TextFlowType" :QUALIFIERS
                ((:DESCRIPTION
                  "An enumeration specifying the type of the text stream supported on this SAP. In some cases this may be a raw character steam, with the characters having no special meaning. However, in other cases it may support a Terminal Mode where some characters have special meanings like backspace, positioning of the character on the screen, etc.")
                 (:VALUE-MAP ("0" "1" "2" "3" "4..32767" "32768..65535"))
                 (:VALUES
                  ("Unknown" "Other" "Character Stream" "Terminal Mode"
                   "DMTF Reserved" "Vendor Specified"))
                 (:MODEL-CORRESPONDENCE
                  ("CIM_TextRedirectionSAP.OtherTextFlowType")))
                :INITFORM NIL)
               (OTHERTEXTFLOWTYPE STRING :CIM-NAME "OtherTextFlowType"
                :QUALIFIERS
                ((:DESCRIPTION
                  "An string describing the TextFlowType when the value of the TextFlowType property is set to 1 = \"Other\".")
                 (:MODEL-CORRESPONDENCE
                  ("CIM_TextRedirectionSAP.TextFlowType")))
                :INITFORM NIL)
...
OperationalStatus replaces the Status property on ManagedSystemElement to provide a consistent approach to enumerations, to address implementation needs for an array property, and to provide a migration path from today's environment to the future. This change was not made earlier because it required the deprecated qualifier. Due to the widespread use of the existing Status property in management applications, it is strongly recommended that providers or instrumentation provide both the Status and OperationalStatus properties. Further, the first value of OperationalStatus should contain the primary status for the element. When instrumented, Status (because it is single-valued) should also provide the primary status of the element.")
               ...
               (STATUSDESCRIPTIONS (ARRAY STRING) :CIM-NAME
                ...
                :INITFORM NIL))
              (:CIM-NAME "CIM_TextRedirectionSAP")
...
(DEFCIM-METHOD REQUESTSTATECHANGE
    ((INSTANCE CIM_TEXTREDIRECTIONSAP)
