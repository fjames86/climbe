...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_TimeService.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_TIMESERVICE (CIM_SERVICE)
...
(DEFCIM-METHOD MANAGETIME
    ((INSTANCE CIM_TIMESERVICE)
     (GETREQUEST BOOLEAN "GetRequest" :IN
      (:DESCRIPTION
       "Indicates whether the request is to get (TRUE) or set (FALSE) the time for the specified element."))
     (TIMEDATA DATETIME "TimeData" :IN :OUT
      (:DESCRIPTION
       "The current date and time. On input, this value is the time that is set on the specified element, if the GetRequest input parameter is TRUE. Otherwise, the input time is ignored. On output, this value is the time as it is known by the element."))
     (MANAGEDELEMENT "CIM_ManagedElement" "ManagedElement" :REQUIRED :IN
      (:DESCRIPTION
       "A reference to the ManagedElement whose time is retrieved or set. This reference is necessary because the TimeService might apply to multiple elements simultaneously. The referenced element must be associated with the Service using the ServiceAffectsElement relationship.")))
    ((:CIM-NAME "ManageTime")
     (:QUALIFIERS
      (:DESCRIPTION
