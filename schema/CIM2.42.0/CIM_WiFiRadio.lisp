...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/CIM_WiFiRadio.xml"

(in-package #:climbe)


(DEFCIM-CLASS CIM_WIFIRADIO (CIM_LOGICALDEVICE)
              ((SIGNALSTRENGTH UINT32 :CIM-NAME "SignalStrength" :QUALIFIERS
                ((:DESCRIPTION "The received signal strength, in -dBm.")
                 (:P-UNIT "decibel(m) * -1"))
                :INITFORM NIL)
               (SIGNALNOISE UINT32 :CIM-NAME "SignalNoise" :QUALIFIERS
                ((:DESCRIPTION "The received signal noise, in -dBm.")
                 (:P-UNIT "decibel(m) * -1"))
                :INITFORM NIL)
               (CHANNEL UINT16 :CIM-NAME "Channel" :QUALIFIERS
                ((:DESCRIPTION
                  "Channel shall contain the channel number currently in use by the WiFiRadio to communicate with the station with which it is associated. Each channel number is associated with a particular signal frequency band."))
                :INITFORM NIL)
               (FREQUENCY UINT32 :CIM-NAME "Frequency" :QUALIFIERS
                ((:DESCRIPTION
                  "The frequency, in Megahertz, of the center frequency of the band (channel) currently in use by the WiFiRado.")
                 (:P-UNIT "hertz * 10^6"))
                :INITFORM NIL)
