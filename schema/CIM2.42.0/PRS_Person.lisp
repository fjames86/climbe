...
;; Compiled from #P"/home/frank/quicklisp/local-projects/climbe/schema/CIM2.42.0/PRS_Person.xml"

(in-package #:climbe)


(DEFCIM-CLASS PRS_PERSON (PRS_CONTACTITEM)
              ((FIRSTNAME STRING :CIM-NAME "FirstName" :QUALIFIERS
                ((:DESCRIPTION "Contact's first name.")) :INITFORM NIL)
               (LASTNAME STRING :CIM-NAME "LastName" :QUALIFIERS
                ((:DESCRIPTION "Contact's last name.")) :INITFORM NIL)
               (SALUTATION STRING :CIM-NAME "Salutation" :QUALIFIERS
                ((:DESCRIPTION
                  "How the contact is addressed, i.e. Dr., Mr., Ms., Mrs., etc."))
                :INITFORM NIL)
               (TITLE STRING :CIM-NAME "Title" :QUALIFIERS
                ((:DESCRIPTION "Contact's business title.")) :INITFORM NIL)
               (CONTACTTYPE STRING :CIM-NAME "ContactType" :QUALIFIERS
                ((:DESCRIPTION
                  "Description of the contact. For example, they may have responsibilities not apparent from their title."))
                :INITFORM NIL)
               (COMMUNICATIONMODE UINT8 :CIM-NAME "CommunicationMode"
                :QUALIFIERS
                ((:DESCRIPTION
                  "Contact's preferred communication method. See the PRS_Location.LocationType property.")
                 (:VALUE-MAP ("0" "1" "2" "3" "4" "254" "255"))
                 (:VALUES
                  ("Primary_Voice" "Alternate_Voice" "Fax" "Email" "URL"
                   "Other" "Unknown")))
                :INITFORM NIL)
